...
- `git log` shows the commit history of the current branch, and it displays the commit messages, author, date, and other relevant information.
    - `git log --oneline --decorate --graph`: this command will show a compact log with _one line per commit_, displaying only the _commit hash_ and the _commit **message**_. The `--decorat`e option adds information about _branches_ and _tags_, and the `--graph` option draws a text-based _graph_ to represent _commit history_. 
